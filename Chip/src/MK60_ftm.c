...
FTM正交ＡＢ相测速初始化函数，野火的函数有问题，
移植龙邱的编码器初始化例程

ｂｙ－－王明龙

*****************************/
void ftm_quad_init2(void)   
{
    PORTA_PCR8  = PORT_PCR_MUX(6); // 设置引脚 A8引脚为FTM1_PHA功能
    PORTA_PCR9  = PORT_PCR_MUX(6); // 设置引脚 A9引脚为FTM1_PHB功能
    ...
    FTM1_QDCTRL|=FTM_QDCTRL_QUADEN_MASK;//启用FTM1正交模式
    FTM1_MODE |= FTM_MODE_FTMEN_MASK;//FTM1EN=1	
    
    FTM2_QDCTRL|=FTM_QDCTRL_QUADEN_MASK;//启用FTM2正交模式，我们是正交ＡＢ相模式，不是正交解码模式
